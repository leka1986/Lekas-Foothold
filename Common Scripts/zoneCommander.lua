...
function measureDistanceZoneToZone(zoneA,zoneB)
	...
	local function cleanName(s)
		s = tostring(s or "")
		return (s:gsub("%s*#%s*%d+", ""):gsub("%s+Fixed", ""):gsub("%s%s+", " "):gsub("^%s+", ""):gsub("%s+$", ""))
	end
    for i,v in ipairs(self.zones) do
        ...
        for i2,v2 in pairs(v.built) do
            ...
            if gr then
				...
				if groupStatus and gr:isExist() and (gr:getSize() or 0) > 0 then
					local entry = { name = cleanName(v2), kind = "group" }
					local initialSize = gr.getInitialSize and gr:getInitialSize() or nil
					local currentSize = gr:getSize() or 0
					if initialSize and initialSize > 0 and currentSize < initialSize then
						entry.damaged = true
					end
					groupStatus[#groupStatus + 1] = entry
				end
			elseif groupStatus then
				local st = StaticObject.getByName(v2)
				if st then
					groupStatus[#groupStatus + 1] = { name = cleanName(v2), kind = "static" }
				end
            end
        end
